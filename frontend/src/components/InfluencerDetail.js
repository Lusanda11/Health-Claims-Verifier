...
import { Container, Card, ListGroup, Spinner, Alert } from "react-bootstrap";
import { searchInfluencer } from "../api/influencerApi";
...
const InfluencerDetail = () => {
  ...
  useEffect(() => {
    const fetchInfluencer = async () => {
      try {
        setLoading(true);
        const data = await searchInfluencer(id);
    ...
    <Container className="mt-5">
      <Card className="shadow-lg">
        <Card.Body className="text-center">
          ...
          <Card.Text className="text-muted">{influencer.bio || "No bio available."}</Card.Text>
          <Card.Text>
            <strong>👥 Followers:</strong>{" "}
      ...
      <h3 className="mt-4">Recent Health-Related Claims</h3>
      {Array.isArray(influencer.claims) && influencer.claims.length > 0 ? (
