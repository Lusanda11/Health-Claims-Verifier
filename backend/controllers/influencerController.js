...
const getAllInfluencers = async (req, res) => {
  ...
  try {
    const BEARER_TOKEN = process.env.TWITTER_BEARER_TOKEN;
    if (!BEARER_TOKEN) throw new Error("Missing Twitter API Bearer Token");

    const usernames = ["dr_asha", "nutrition_facts", "DrGreger", "mindbodygreen", "drhyman"];
    let retries = 3;
    let response;
...
// ----------------- Search Single Influencer -----------------
const searchInfluencer = async (req, res) => {
  try {
    let { username } = req.query;

    // 🔹 Return 400 if username missing
...
module.exports = { getAllInfluencers, searchInfluencer };
